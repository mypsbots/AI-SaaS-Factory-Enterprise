---
id: KB-ENGINEERING_PRINCIPLES
framework: AI SaaS Factory Enterprise (ASFE)
owner: Enterprise Architect
department: Knowledge
version: 1.0.0
status: Active
criticality: Medium
review_frequency: Annually
classification: Internal
created: 2026-06-25
updated: 2026-06-25
---

# KNOWLEDGE — Engineering Principles

> The cross-cutting principles every ASFE artefact must uphold.

---

## 1. Summary

The cross-cutting principles every ASFE artefact must uphold.

---

## 2. Context

Curated reference knowledge for teams building SaaS products.

---

## 3. Key Concepts

- Security and Privacy by Design
- Accessibility by Default
- Cloud Native and API First
- Observability First
- Zero Trust and Least Privilege
- Documentation and Infrastructure as Code

---

## 4. Guidance

- Apply principles as defaults, not exceptions.
- Make principles measurable via gates and KPIs.
- Resolve conflicts in favour of security, privacy, and safety.

---

## 5. Pitfalls

- Treating principles as aspirational slogans.
- Bolting on security/accessibility after build.
- Principles without enforcement.

---

## 6. Related Documents

- Specification (../ASFE_SPECIFICATION.md)
- Relevant Rules, Skills, and Checklists in this repository.

---

## 7. References

- Official standards and primary sources (see Standards Index).

---

## 8. Metadata

- **Knowledge ID:** KB-ENGINEERING_PRINCIPLES
- **Owner:** Enterprise Architect
- **Version:** 1.0.0
- **Status:** Active
- **Review Frequency:** Annually
- **Created:** 2026-06-25
- **Updated:** 2026-06-25
