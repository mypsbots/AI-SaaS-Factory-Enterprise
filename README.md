# AI SaaS Factory Enterprise (ASFE)

**A reusable AI engineering operating system for building production-grade SaaS products with AI assistants and human engineers.**

Version: 1.0.0 · Generated: 2026-06-25

ASFE is the single source of truth governing product engineering, AI engineering,
architecture, security, DevSecOps, quality, cloud, operations, growth, and governance.
Every document is **Documentation-as-Code**: generated from versioned data tables and
the master prompts in [`_generators/`](_generators/), regenerable via
[`_build/generate_asfe.py`](_build/generate_asfe.py).

> **Reusing a single file?** Individual Skills, Rules, Checklists, Playbooks, Templates,
> and Knowledge articles are written to be framework-neutral, so you can lift any one of
> them into your own project. The ASFE brand lives only here, in the
> specification, and in each document's front-matter `framework` field.

---

## Contents

| Type | Count |
| --- | --- |
| Specialist Skills | 37 |
| Engineering Rules | 24 |
| Quality Checklists | 18 |
| Operational Playbooks | 12 |
| Document Templates | 10 |
| Knowledge Articles | 6 |
| Department Indexes | 28 |
| **Total Markdown documents** | **137** |

---

## Repository Structure

| Folder | Department | Focus |
| --- | --- | --- |
| `00_Governance` | Governance | Enterprise governance, document lifecycle, RACI, and quality-gate ownership. |
| `01_Executive` | Executive | Strategy, vision, OKRs, and cross-department alignment. |
| `02_Product` | Product | Product strategy, requirements, roadmap, and documentation. |
| `03_UX` | UX | User research, interaction design, and usability. |
| `04_Frontend` | Frontend | Client applications, design system, and accessibility delivery. |
| `05_Backend` | Backend | Services, business logic, and server-side reliability. |
| `06_AI` | AI | AI/LLM engineering, prompting, safety, and evaluation. |
| `07_RAG` | RAG | Retrieval-augmented generation pipelines and knowledge grounding. |
| `08_Data` | Data | Data modelling, pipelines, governance, and analytics. |
| `09_Database` | Database | Database architecture, schema design, and migrations. |
| `10_API` | API | API design, contracts, versioning, and governance. |
| `11_Cloud` | Cloud | Cloud architecture, networking, and well-architected design. |
| `12_DevOps` | DevOps | CI/CD, infrastructure as code, and automation. |
| `13_DevSecOps` | DevSecOps | Security automation across the delivery pipeline. |
| `14_Security` | Security | Application and platform security engineering. |
| `15_Privacy` | Privacy | Data protection, privacy engineering, and DPIAs. |
| `16_Compliance` | Compliance | Audit readiness, controls, and certifications. |
| `17_QA` | QA | Quality engineering, test strategy, and accessibility QA. |
| `18_Performance` | Performance | Performance engineering and capacity planning. |
| `19_Observability` | Observability | Telemetry, monitoring, SLOs, and alerting. |
| `20_SEO` | SEO | SEO, AEO, and GEO discoverability engineering. |
| `21_Marketing` | Marketing | Product and growth marketing. |
| `22_Sales` | Sales | Sales enablement and revenue operations. |
| `23_CustomerSuccess` | Customer Success | Onboarding, retention, and customer health. |
| `24_Finance` | Finance | SaaS finance, billing, and payments. |
| `25_Legal` | Legal | Contracts, licensing, and regulatory counsel. |
| `26_Release` | Release | Release management and change control. |
| `27_Operations` | Operations | Site reliability and production operations. |

Cross-cutting asset folders: `Skills/`, `Rules/`, `Checklists/`, `Playbooks/`, `Templates/`, `Knowledge/`.

---

## Guiding Principles

- Security by Design
- Privacy by Design
- Accessibility by Default
- Performance First
- Observability First
- API First
- Cloud Native
- Zero Trust
- Principle of Least Privilege
- Secure Defaults
- Testability
- Documentation as Code
- Automation First
- Backward Compatibility

---

## Document Types

1. **SKILLS.md** — specialist responsibilities, authority, and standards.
2. **RULES.md** — mandatory, enforceable engineering standards.
3. **CHECKLIST.md** — auditable quality gates.
4. **PLAYBOOK.md** — repeatable operational procedures.
5. **TEMPLATE.md** — reusable document scaffolds.
6. **KNOWLEDGE.md** — curated technical reference.

---

## Quality Gates

Every feature flows through:
Idea → Product → Architecture → Security → AI → Backend → Frontend → QA → Performance → Accessibility → SEO/AEO/GEO → Release → Production.

---

## Skills

- [Backend Engineer](Skills/Backend_Engineer.md) — Backend
- [Frontend Engineer](Skills/Frontend_Engineer.md) — Frontend
- [Database Architect](Skills/Database_Architect.md) — Database
- [API Architect](Skills/API_Architect.md) — API
- [Cloud Architect](Skills/Cloud_Architect.md) — Cloud
- [DevOps Engineer](Skills/DevOps_Engineer.md) — DevOps
- [DevSecOps Engineer](Skills/DevSecOps_Engineer.md) — DevSecOps
- [Site Reliability Engineer](Skills/Site_Reliability_Engineer.md) — Operations
- [Observability Engineer](Skills/Observability_Engineer.md) — Observability
- [AI Engineer](Skills/AI_Engineer.md) — AI
- [Prompt Engineer](Skills/Prompt_Engineer.md) — AI
- [RAG Engineer](Skills/RAG_Engineer.md) — RAG
- [AI Safety Engineer](Skills/AI_Safety_Engineer.md) — AI
- [AI Evaluation Engineer](Skills/AI_Evaluation_Engineer.md) — AI
- [Security Engineer](Skills/Security_Engineer.md) — Security
- [Privacy Engineer](Skills/Privacy_Engineer.md) — Privacy
- [Compliance Specialist](Skills/Compliance_Specialist.md) — Compliance
- [Product Manager](Skills/Product_Manager.md) — Product
- [UX Researcher](Skills/UX_Researcher.md) — UX
- [UI UX Designer](Skills/UI_UX_Designer.md) — UX
- [Technical Writer](Skills/Technical_Writer.md) — Product
- [QA Engineer](Skills/QA_Engineer.md) — QA
- [Accessibility Specialist](Skills/Accessibility_Specialist.md) — QA
- [Performance Engineer](Skills/Performance_Engineer.md) — Performance
- [SEO AEO GEO Specialist](Skills/SEO_AEO_GEO_Specialist.md) — SEO
- [Product Marketing](Skills/Product_Marketing.md) — Marketing
- [Growth Engineer](Skills/Growth_Engineer.md) — Marketing
- [Digital Marketing](Skills/Digital_Marketing.md) — Marketing
- [SaaS Finance](Skills/SaaS_Finance.md) — Finance
- [Payments Engineer](Skills/Payments_Engineer.md) — Finance
- [Customer Success](Skills/Customer_Success.md) — Customer Success
- [Release Manager](Skills/Release_Manager.md) — Release
- [Enterprise Architect](Skills/Enterprise_Architect.md) — Governance
- [Chief Technology Officer](Skills/Chief_Technology_Officer.md) — Executive
- [Data Engineer](Skills/Data_Engineer.md) — Data
- [Sales Engineer](Skills/Sales_Engineer.md) — Sales
- [Legal Counsel](Skills/Legal_Counsel.md) — Legal

## Rules

- [API Standards](Rules/API_Standards.md)
- [Coding Standards](Rules/Coding_Standards.md)
- [Security Standards](Rules/Security_Standards.md)
- [Authentication And Authorization](Rules/Authentication_And_Authorization.md)
- [Data Protection And Privacy](Rules/Data_Protection_And_Privacy.md)
- [Logging And Observability](Rules/Logging_And_Observability.md)
- [Error Handling](Rules/Error_Handling.md)
- [Testing Standards](Rules/Testing_Standards.md)
- [Git And Version Control](Rules/Git_And_Version_Control.md)
- [CI CD Standards](Rules/CI_CD_Standards.md)
- [Infrastructure As Code](Rules/Infrastructure_As_Code.md)
- [Secrets Management](Rules/Secrets_Management.md)
- [Database Standards](Rules/Database_Standards.md)
- [Accessibility Standards](Rules/Accessibility_Standards.md)
- [Performance Budgets](Rules/Performance_Budgets.md)
- [AI LLM Usage](Rules/AI_LLM_Usage.md)
- [Prompt Engineering Standards](Rules/Prompt_Engineering_Standards.md)
- [RAG Standards](Rules/RAG_Standards.md)
- [Dependency Management](Rules/Dependency_Management.md)
- [Documentation Standards](Rules/Documentation_Standards.md)
- [Release Management](Rules/Release_Management.md)
- [Incident Management](Rules/Incident_Management.md)
- [Compliance Standards](Rules/Compliance_Standards.md)
- [Frontend Standards](Rules/Frontend_Standards.md)

## Checklists

- [Production Release Checklist](Checklists/Production_Release_Checklist.md)
- [Security Review Checklist](Checklists/Security_Review_Checklist.md)
- [Architecture Review Checklist](Checklists/Architecture_Review_Checklist.md)
- [API Review Checklist](Checklists/API_Review_Checklist.md)
- [Code Review Checklist](Checklists/Code_Review_Checklist.md)
- [AI Prompt Review Checklist](Checklists/AI_Prompt_Review_Checklist.md)
- [RAG Evaluation Checklist](Checklists/RAG_Evaluation_Checklist.md)
- [Performance Audit Checklist](Checklists/Performance_Audit_Checklist.md)
- [Accessibility Audit Checklist](Checklists/Accessibility_Audit_Checklist.md)
- [Incident Response Checklist](Checklists/Incident_Response_Checklist.md)
- [GDPR Readiness Checklist](Checklists/GDPR_Readiness_Checklist.md)
- [Stripe Integration Checklist](Checklists/Stripe_Integration_Checklist.md)
- [SEO AEO GEO Review Checklist](Checklists/SEO_AEO_GEO_Review_Checklist.md)
- [Database Migration Checklist](Checklists/Database_Migration_Checklist.md)
- [Penetration Test Readiness Checklist](Checklists/Penetration_Test_Readiness_Checklist.md)
- [Data Privacy Checklist](Checklists/Data_Privacy_Checklist.md)
- [Observability Readiness Checklist](Checklists/Observability_Readiness_Checklist.md)
- [Disaster Recovery Checklist](Checklists/Disaster_Recovery_Checklist.md)

## Playbooks

- [Incident Response Playbook](Playbooks/Incident_Response_Playbook.md)
- [Production Deployment Playbook](Playbooks/Production_Deployment_Playbook.md)
- [Rollback Playbook](Playbooks/Rollback_Playbook.md)
- [Disaster Recovery Playbook](Playbooks/Disaster_Recovery_Playbook.md)
- [On Call Playbook](Playbooks/On_Call_Playbook.md)
- [Security Incident Playbook](Playbooks/Security_Incident_Playbook.md)
- [Data Breach Response Playbook](Playbooks/Data_Breach_Response_Playbook.md)
- [Performance Tuning Playbook](Playbooks/Performance_Tuning_Playbook.md)
- [Feature Launch Playbook](Playbooks/Feature_Launch_Playbook.md)
- [Database Migration Playbook](Playbooks/Database_Migration_Playbook.md)
- [Cost Optimization Playbook](Playbooks/Cost_Optimization_Playbook.md)
- [Customer Escalation Playbook](Playbooks/Customer_Escalation_Playbook.md)

## Templates

- [SKILLS Template](Templates/SKILLS_Template.md)
- [RULES Template](Templates/RULES_Template.md)
- [CHECKLIST Template](Templates/CHECKLIST_Template.md)
- [PLAYBOOK Template](Templates/PLAYBOOK_Template.md)
- [KNOWLEDGE Template](Templates/KNOWLEDGE_Template.md)
- [ADR Template](Templates/ADR_Template.md)
- [PRD Template](Templates/PRD_Template.md)
- [RFC Template](Templates/RFC_Template.md)
- [Postmortem Template](Templates/Postmortem_Template.md)
- [Runbook Template](Templates/Runbook_Template.md)

## Knowledge

- [Engineering Principles](Knowledge/Engineering_Principles.md)
- [Glossary](Knowledge/Glossary.md)
- [Technology Radar](Knowledge/Technology_Radar.md)
- [Standards Index](Knowledge/Standards_Index.md)
- [Security Reference Architecture](Knowledge/Security_Reference_Architecture.md)
- [AI Engineering Reference](Knowledge/AI_Engineering_Reference.md)

---

## Regenerating

```bash
python3 _build/generate_asfe.py
```

All content is deterministic and reproducible from `_build/` data tables.

---

## License & Status

Internal engineering operating system. See [`ASFE_SPECIFICATION.md`](ASFE_SPECIFICATION.md) for governance.
