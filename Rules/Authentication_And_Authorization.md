...
**All access MUST be authenticated and authorized server-side using standards-based protocols.**
...
Mandatory for All access control. Priority: **Critical**. Owner: **Security Engineer**.

---

## 10. Mandatory Requirements (MUST)

- MUST use OAuth 2.1 / OIDC for authentication.
- MUST enforce authorization on the server for every request.
- MUST apply role/attribute-based access control with least privilege.
- MUST expire and rotate tokens; support revocation.

---

## 11. Recommended Practices (SHOULD)

- SHOULD use short-lived access tokens with refresh rotation.
- SHOULD log authn/authz decisions for audit.

---

## 12. Optional Enhancements (MAY)

- MAY support step-up authentication for sensitive actions.
...
- Trusting client-supplied roles or IDs.
...
- Request a time-boxed exception from the Security Engineer with justification and risk assessment.
...
- [ ] Does the change satisfy: Use OAuth 2.1 / OIDC for authentication.
- [ ] Does the change satisfy: Enforce authorization on the server for every request.
- [ ] Does the change satisfy: Apply role/attribute-based access control with least privilege.
- [ ] Does the change satisfy: Expire and rotate tokens; support revocation.
...
- **Rule ID:** RULE-AUTHENTICATION_AND_AUTHORIZATION
- **Owner:** Security Engineer
- **Category:** Security
- **Version:** 1.0.0
- **Status:** Active
