...
**All APIs MUST be contract-first, versioned, consistent, and secure by default.**
...
Mandatory for All APIs. Priority: **Critical**. Owner: **API Architect**.

---

## 10. Mandatory Requirements (MUST)

- MUST publish an OpenAPI 3.1 contract as the source of truth before implementation.
- MUST use explicit versioning and a documented deprecation policy.
- MUST return errors using RFC 9457 problem-details.
- MUST authenticate and authorize every endpoint; default deny.
- MUST validate and constrain all inputs; enforce pagination limits.

---

## 11. Recommended Practices (SHOULD)

- SHOULD provide idempotency keys for mutating endpoints.
- SHOULD offer consistent filtering, sorting, and pagination conventions.
- SHOULD publish consumer-driven contract tests.

---

## 12. Optional Enhancements (MAY)

- MAY provide GraphQL or gRPC alongside REST where justified.
- MAY expose an AsyncAPI contract for event streams.
...
- Leaking internal models as responses.
...
- Request a time-boxed exception from the API Architect with justification and risk assessment.
...
- [ ] Does the change satisfy: Publish an OpenAPI 3.1 contract as the source of truth before implementation.
- [ ] Does the change satisfy: Use explicit versioning and a documented deprecation policy.
- [ ] Does the change satisfy: Return errors using RFC 9457 problem-details.
- [ ] Does the change satisfy: Authenticate and authorize every endpoint; default deny.
- [ ] Does the change satisfy: Validate and constrain all inputs; enforce pagination limits.
