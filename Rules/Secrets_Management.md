...
Establish a reusable, enforceable standard for **Secrets Management** across all ASFE SaaS products.
...
**Secrets MUST be centrally managed, encrypted, rotated, and never committed.**
...
Mandatory for All secrets. Priority: **Critical**. Owner: **Security Engineer**.

---

## 10. Mandatory Requirements (MUST)

- MUST store secrets in a dedicated secrets manager.
- MUST encrypt secrets at rest and in transit.
- MUST rotate secrets and credentials regularly.
- MUST scan code and history for leaked secrets.

---

## 11. Recommended Practices (SHOULD)

- SHOULD prefer short-lived, dynamically issued credentials.
- SHOULD scope secrets to least privilege.

---

## 12. Optional Enhancements (MAY)

- MAY use workload identity to avoid static secrets.
...
- Secrets in code, env files, or images.
...
- [ ] Does the change satisfy: Store secrets in a dedicated secrets manager.
- [ ] Does the change satisfy: Encrypt secrets at rest and in transit.
- [ ] Does the change satisfy: Rotate secrets and credentials regularly.
- [ ] Does the change satisfy: Scan code and history for leaked secrets.
...
- **Rule ID:** RULE-SECRETS_MANAGEMENT
- **Owner:** Security Engineer
- **Category:** Security
- **Version:** 1.0.0
- **Status:** Active
