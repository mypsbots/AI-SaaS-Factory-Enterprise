...
Establish a reusable, enforceable standard for **Security Standards** across all ASFE SaaS products.
...
**Systems MUST be secure by design, applying zero trust and least privilege.**
...
Mandatory for All systems. Priority: **Critical**. Owner: **Security Architect**.

---

## 10. Mandatory Requirements (MUST)

- MUST threat model features before implementation.
- MUST encrypt data in transit (TLS 1.2+) and at rest.
- MUST enforce least-privilege access and short-lived credentials.
- MUST validate, sanitise, and encode all untrusted input/output.
- MUST pass SAST/DAST/SCA gates with no unresolved criticals.

---

## 11. Recommended Practices (SHOULD)

- SHOULD adopt MFA and phishing-resistant auth.
- SHOULD rotate secrets automatically.
- SHOULD conduct periodic penetration testing.

---

## 12. Optional Enhancements (MAY)

- MAY use hardware-backed key storage (HSM/KMS).
...
- Request a time-boxed exception from the Security Architect with justification and risk assessment.
...
- [ ] Does the change satisfy: Threat model features before implementation.
- [ ] Does the change satisfy: Encrypt data in transit (TLS 1.2+) and at rest.
- [ ] Does the change satisfy: Enforce least-privilege access and short-lived credentials.
- [ ] Does the change satisfy: Validate, sanitise, and encode all untrusted input/output.
- [ ] Does the change satisfy: Pass SAST/DAST/SCA gates with no unresolved criticals.
...
- **Rule ID:** RULE-SECURITY_STANDARDS
- **Owner:** Security Architect
- **Category:** Security
- **Version:** 1.0.0
- **Status:** Active
