---
id: RULE-LOGGING_AND_OBSERVABILITY
...
Establish a reusable, enforceable standard for **Logging And Observability** across all ASFE SaaS products.
...
**Services MUST emit structured, correlated, privacy-safe telemetry.**
...
Mandatory for All services. Priority: **High**. Owner: **Observability Engineer**.

---

## 10. Mandatory Requirements (MUST)

- MUST emit structured logs with correlation/trace IDs.
- MUST expose metrics aligned to SLIs.
- MUST propagate distributed traces (OpenTelemetry).
- MUST never log secrets or unmasked personal data.

---

## 11. Recommended Practices (SHOULD)

- SHOULD define retention and sampling to manage cost.
- SHOULD alert on symptoms tied to SLOs.

---

## 12. Optional Enhancements (MAY)

- MAY adopt exemplars linking metrics to traces.
...
- Request a time-boxed exception from the Observability Engineer with justification and risk assessment.
...
- [ ] Does the change satisfy: Emit structured logs with correlation/trace IDs.
- [ ] Does the change satisfy: Expose metrics aligned to SLIs.
- [ ] Does the change satisfy: Propagate distributed traces (OpenTelemetry).
- [ ] Does the change satisfy: Never log secrets or unmasked personal data.
...
- **Rule ID:** RULE-LOGGING_AND_OBSERVABILITY
- **Owner:** Observability Engineer
- **Category:** Operations
- **Version:** 1.0.0
- **Status:** Active
