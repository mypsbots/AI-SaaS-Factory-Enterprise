...
Establish a reusable, enforceable standard for **Coding Standards** across all ASFE SaaS products.
...
**Code MUST be readable, tested, secure, and consistently formatted.**
...
Mandatory for All source code. Priority: **High**. Owner: **Principal Engineer**.

---

## 10. Mandatory Requirements (MUST)

- MUST pass automated linting and formatting in CI.
- MUST include tests for new and changed behaviour.
- MUST avoid hardcoded secrets and credentials.
- MUST handle errors explicitly; never silently swallow them.

---

## 11. Recommended Practices (SHOULD)

- SHOULD keep functions small and single-purpose.
- SHOULD prefer composition and clear naming over cleverness.
- SHOULD document non-obvious intent, not the obvious.

---

## 12. Optional Enhancements (MAY)

- MAY adopt language-specific idioms documented per repo.
...
- [ ] Does the change satisfy: Pass automated linting and formatting in CI.
- [ ] Does the change satisfy: Include tests for new and changed behaviour.
- [ ] Does the change satisfy: Avoid hardcoded secrets and credentials.
- [ ] Does the change satisfy: Handle errors explicitly; never silently swallow them.
